...
#Same methods as before but this time do a different stat to weight by (deaths proportion = (group deaths/state deaths))
```{r}
# Black mortality
...
blac_mort <- black_male_mort %>% 
  full_join(black_female_mort, by = c("State", "Year"))
```
...
white_female_mortality <- mort_state_race %>% 
  filter(Race == "White", Gender.Code == "F") %>% 
  group_by(State, Year) %>% 
  mutate(population_proportion = (Group_Population/State_Population), deaths_proportion = (Group_Deaths/State_Deaths), white_female_mortality = (Group_Deaths/State_Poppulation) * 100000, white_female_mortality_weighted = white_female_mortality/population_proportion, white_female_mortality_weighted2 = white_female_mortality*death_proportion)
```
